...
| **Ruti** | Consultora pedagógica | Orientação editorial, padrões pedagógicos, revisão de qualidade |
...
|---|---|---|---|---|
| 1 | **Princípios Editoriais** (Content Playbook) | Transversal | Elton + Ruti | Princípios editoriais inegociáveis. Escopo positivo e negativo da base ("o que somos / o que não somos"). |
| 2 | **Matriz de Voz e Tom** (Voice & Tone Matrix) | Pilares 3, 10 | Ruti + Juliana | Voz constante da base + variações de tom por contexto do leitor e tipo de artigo. |
| 3 | **Vocabulário Controlado** (Controlled Vocabulary) | Pilares 3, 4 | Ruti + Elton + Juliana | Vocabulário padronizado: termos preferidos, sinônimos rejeitados, mapeamento CEFOR ↔ Moodle ↔ MEC ↔ popular. |
| 4 | **Biblioteca de Padrões de Conteúdo** (Content Patterns Library) | Pilares 3, 10 | Elton + Ruti | Padrões reutilizáveis de estrutura textual: passo a passo com checkpoint, problema→solução, quando usar/não usar, analogia, antes/depois, FAQ invertido, resumo progressivo. |
| 5 | **Rubrica de Qualidade** (Quality Scorecard) | Pilares 7, 10 | Ruti + Juliana + Elton | Rubrica com 7 dimensões ponderadas (acionabilidade, escaneabilidade, precisão, completude, clareza, conformidade, frescor). Score mínimo para publicação. Também usado na triagem dos ~60 artigos existentes. |
| 6 | **Padrões de Legibilidade** (Readability Standards) | Pilares 3, 6 | Ruti + Elton | Métricas objetivas de legibilidade: palavras por frase, frases por parágrafo, índice Flesch PT-BR, voz passiva, jargão. Limites por tipo de artigo. |
| 7 | **Mapa de Modelos Mentais do Público** (Audience Mental Model Map) | Pilares 3, 10 | Ruti + Juliana | Perfis mentais do público (professor migrante, mediador experiente, coordenador, novato, técnico TI). Momentos de busca. Vocabulário natural. |
...
| 9 | **Operação de Conteúdo** (ContentOps Charter) | Pilares 7, 10 | Elton + Marcos | Capacidade operacional real: ~4-6 artigos/mês. Cadência de revisão por tipo. Fluxo de produção com IA (prompt → geração → revisão Juliana → revisão Ruti → revisão Elton → scorecard → publicação). SLAs. |
...
| **Arquitetura Trilha/Percurso** | Pilar 4 | ✅ Validado | Elton + Marquito + Ruti | Trilha = jornada atômica de artigos (começo, meio, fim). Percurso = conjunto de trilhas + possíveis artigos soltos. Um artigo pode estar em múltiplas trilhas. |
...
**Critério de conclusão:** Cada entrega documentada e aprovada por Elton + Marcos. Ruti valida as camadas 1-7 do Content System. Todos os documentos de referência atualizados.
...
**Responsável:** Juliana (avaliação) + Ruti (validação pedagógica) + Elton (score final)

### Etapa 4.2 — Produção e Reescrita

| Entrega | Responsável | Descrição |
|---|---|---|
| **Reescrita dos artigos** | Juliana + Ruti + IA | Seguindo o fluxo da Operação de Conteúdo (Camada 9): prompt template → geração IA → revisão Juliana → revisão Ruti → revisão Elton → rubrica ≥ 3.5 → publicação |
...
| **Vocabulário Controlado aplicado** | Juliana + Ruti | Termos padronizados conforme Vocabulário Controlado (Camada 3). Termos Moodle na primeira menção + termo pedagógico entre parênteses. |
...
| **Organização em percursos** | Juliana + Ruti | Artigos organizados nas trilhas/percursos definidos na taxonomia |
...
|---|---|---|---|
| **Produção de artigos novos** | Contínua (~4-6/mês) | Juliana + Ruti + IA | Conforme fluxo da Operação de Conteúdo (Camada 9) |
| **Revisão periódica obrigatória** | Conforme cadência por tipo | Juliana + Ruti | Tutoriais: 6 meses. Troubleshooting: 3 meses. Conceituais: 12 meses. |
...
| **Atualização do Vocabulário Controlado** | Contínua | Ruti + Juliana | Novos termos do público registrados conforme surgem em atendimentos e e-mails. |
...
|---|---|---|
| Triagem dos artigos por rubrica (4.1) | Assim que Fase 1 fechar | Juliana + Ruti |
| Reescrita de artigos em documentos (4.2) | Assim que Fase 1 fechar | Juliana + Ruti + IA |
| Publicação no WordPress (4.2) | Quando Fase 3 estiver funcional | Juliana + Elton |

Isso significa que **Juliana e Ruti não ficam paradas** esperando o WordPress. A triagem e a reescrita em documentos podem acontecer ao longo de toda a Fase 2 e 3.
...
|---|---|---|
| Chatbot/Tutor IA (evolução Sofia) | Ruti + Marcos | V2 — após base estabilizada |
| Gamificação (badges, ranking, prêmio anual) | Marcos + Juliana + Ruti | V3 — após comunidade ativa |
...
| Trilhas com certificação | Ruti | V2/V3 |
| Integração profunda com Moodle (LTI/embed) | Juliana + Marcos | V2 |
| Percurso por perfil (mediador, professor) | Ruti | V1.5 — quick win pós-lançamento |
