...
**Tokens** espelham `drafts/kit-visual.html`. **Componentes e regras** rastreiam as Decisões 1-32 (`_config/decisoes.md`). **Interfaces de referência:** os templates base em `stages/02-design-system/drafts/prototipos-paginas/` (`base-artigo.html`, `base-inicio.html`, `base-percurso.html` e `base-trilha.html`), com `base-artigo.html` como referência principal de página de artigo (Decisão 22).
